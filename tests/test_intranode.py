...
TEST_CONFIGS = [
    TestConfig(previous=p, use_async=a, topk=t) for p, a, t in itertools.product(BOOLS * 3)
]
...
def test_main(
    ...
    def run_single(cfg: TestConfig, should_profile: bool, local_rank: int):
        current_x = cfg.input
        with_topk = cfg.topk
        async_mode = cfg.use_async
        previous_mode = cfg.previous
        ...
        return handle, dispatch_bf16_nvl_recv_bytes, combine_bf16_nvl_send_bytes

    if should_test:
        for cfg in TEST_CONFIGS:
            for input in filter(lambda x: x is not None, (x_pure_rand, x, x_e4m3)):
                cfg.input = input
                run_single(cfg, should_profile=should_profile, local_rank=local_rank)
    ...
    if should_benchmark:
        ...
        nvl_chunk_sizes = nvl_chunk_sizes_dispatch or tuple(range(4, 33, 2)) + (0,)
        # Tune dispatch performance
        best_dispatch_results = None
        fp8_factor = (1 + 4 / 128) / 2
        for current_x in filter(lambda elem: elem is not None, (x_e4m3, x)):
            _cfg.input = current_x
            handle, dispatch_bf16_nvl_recv_bytes, combine_bf16_nvl_send_bytes = run_single(
                _cfg, False, local_rank=local_rank
            )
            ...
            for nvl_chunk_size in nvl_chunk_sizes:
                if nvl_chunk_size > 0:
        ...
        best_time, best_results = 1e10, None
        nvl_chunk_sizes = nvl_chunk_sizes_combine or tuple(range(1, 17, 1)) + (0,)
        for nvl_chunk_size in nvl_chunk_sizes:
            if nvl_chunk_size > 0:
                config = deep_ep.Config(num_sms, nvl_chunk_size, nvl_buffer_size)
            else:
        ...
        if local_rank == 0:
            print(
                f"[tuning] Best combine: SMs {best_results[0]}, NVL chunk {best_results[1]}: \
                    {combine_bf16_nvl_send_bytes / 1e9 / best_time:.2f} GB/s (NVL), \
